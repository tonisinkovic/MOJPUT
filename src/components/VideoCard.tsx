...
const VideoCard = ({
  ...
  return (
    <motion.article
      whileHover={showEmbed ? undefined : { scale: 1.02, y: -4 }}
      whileTap={showEmbed ? undefined : { scale: 0.98 }}
      onClick={onClick}
      className={`group rounded-2xl overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 shadow-lg hover:shadow-xl hover:shadow-violet-500/10 dark:hover:shadow-violet-500/5 transition-all duration-300 ${
    ...
    >
      <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900">
        ...
        <div className="pointer-events-none absolute bottom-2 right-2 px-2 py-0.5 rounded-md bg-black/70 text-white text-xs font-medium">
          {duration}
        </div>
        {isNew && (
          <div className="pointer-events-none absolute top-2 left-2 px-2 py-0.5 rounded-md bg-violet-500 text-white text-xs font-bold">
            NOVO
          </div>
        )}
        {!showEmbed &&
