...
const REQUIRE_SMTP_FOR_VERIFY = (process.env.REQUIRE_SMTP_FOR_VERIFY || "true").toLowerCase() !== "false";

console.log("[config] SMTP_HOST set:", Boolean(process.env.SMTP_HOST));
console.log("[config] SMTP_PORT set:", Boolean(process.env.SMTP_PORT));
console.log("[config] SMTP_USER set:", Boolean(process.env.SMTP_USER));
console.log("[config] SMTP_PASS set:", Boolean(process.env.SMTP_PASS));
...
async function sendVerificationEmail({ to, username, verifyUrl }) {
  ...
  try {
    ...
    console.log("[mail] verification email sent:", {
      to,
...
async function main() {
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    const transport = getMailTransport();
    if (REQUIRE_SMTP_FOR_VERIFY && !transport) {
      return res.status(503).json({
        success: false,
        message:
          "Sustav za slanje potvrde emaila nije konfiguriran (SMTP). Kontaktiraj admina ili konfiguriraj SMTP varijable.",
      });
    }
    ...
    if (!mail.sent) {
      // enforce "must confirm via email": if we can't send, registration fails
      db.prepare("DELETE FROM users WHERE id = ?").run(userId);
      return res.status(502).json({
        success: false,
        message:
          "Ne mogu poslati potvrdu emailom (SMTP). Provjeri SMTP postavke i pokušaj ponovno.",
      });
    }

    return res.json({ success: true, user, verification_required: true });
  });
  ...
  app.post("/api/auth/resend-verification", (req, res) => {
    ...
    const verifyUrl = `${APP_ORIGIN}/prijava?verify=${verifyToken}`;
    sendVerificationEmail({ to: cleanEmail, username: row.username, verifyUrl })
      .then(() => res.json({ success: true }))
      .catch(() => res.status(500).json({ success: false, message: "Ne mogu poslati email. Pokušaj kasnije." }));
  });
  ...
  app.get("/api/forum/conversations/:id/messages", (req, res) => {
